...
function buildPlaceholderReplacer(_mapping) {
  ...
  return function replace(text, escapeForJson) {
    ...
    // Pass 1: flexible regex — catches encoded variants (`\[`, `[`,
    // `&#91;`, etc.) and the plain literal form.
    for (const e of entries) {
    ...
    // Pass 2: dumb literal split/join. If pass-1's regex somehow missed
    // a placeholder that's sitting in the bytes verbatim — e.g. a regex
    // edge case with overlapping alternation, a stale compiled regex,
    // anything — this catches it. Belt-and-suspenders against the
    // recurring "one placeholder per response leaks" bug.
    for (const e of entries) {
      if (out.indexOf(e.placeholder) !== -1) {
        const replacement = escapeForJson ? jsonEscapeForStringValue(e.original) : e.original;
        out = out.split(e.placeholder).join(replacement);
      }
    }
...
async function handleWebUiRequest(req, clientRes, hostname) {
  // Same-origin endpoint: every active placeholder → original mapping.
  // The injected sidebar uses this to do a DOM-level revert pass on
  // assistant messages — guaranteed-correct revert regardless of how
  // the placeholder bytes flowed through the SSE stream, because by
  // the time text hits the DOM it's plain rendered text. Last resort
  // against the long-standing "one placeholder per response leaks" bug.
  //
  // Gated on REVEAL_IN_RESPONSE: when the user has disabled placeholder
  // replacement (`pii-guard placeholder-replace disable`), we MUST NOT
  // hand mappings to the in-page script — otherwise the DOM revert
  // would silently undo the user's choice. Returning an empty list
  // makes the script's lookup table empty, so it leaves placeholders
  // visible like the user asked for.
  if (req.url === '/__pii-guard/mappings') {
    const mappings = REVEAL_IN_RESPONSE
      ? [...persistentMap.values()].map(e => ({
          placeholder: e.placeholder,
          original: e.original,
        }))
      : [];
    const body = JSON.stringify({ mappings, reveal: REVEAL_IN_RESPONSE });
