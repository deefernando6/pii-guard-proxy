...
const DATE_RES = [
  ...
  // Year-first slashes (log timestamps): "2026/05/04" / "2026/05/04 11:03:01"
  /\b\d{4}\/\d{1,2}\/\d{1,2}(?:[\sT]\d{1,2}:\d{2}(?::\d{2})?)?\b/g,
  // Numeric DD/MM/YYYY or MM/DD/YYYY (also DD-MM-YYYY)
  /\b\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}\b/g,
  // Apache / nginx CLF timestamp: "04/May/2026:11:03:01 +0000"
  new RegExp(`\\b\\d{1,2}\\/${MONTHS}\\/\\d{2,4}(?::\\d{1,2}:\\d{2}(?::\\d{2})?)?(?:\\s*[+-]\\d{4})?\\b`, 'g'),
...
function detectSupplementary(text) {
  ...
  for (const re of DATE_RES) out.push(...findAll(text, re, 'DATE', 'date'));
