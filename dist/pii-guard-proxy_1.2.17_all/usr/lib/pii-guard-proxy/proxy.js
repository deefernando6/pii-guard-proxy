...
const { recordWebAnonymization, getRecentEvents, maskOriginal, injectSidebar } = require('./lib/web-injector');
...
async function handleWebUiRequest(req, clientRes, hostname) {
  ...
  // recent anonymization events. We MITM the LLM host, so the page can
  // fetch this as a relative URL with no CORS / extra-origin gymnastics.
  if (req.url === '/__pii-guard/recent') {
    const events = getRecentEvents(50).map(e => ({
      ts: e.ts,
